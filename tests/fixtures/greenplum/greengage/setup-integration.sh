...
SQL
  ...
  canonical_distribution_policy="$(query_scalar "${FIXTURE_DATABASE}" \
    "SELECT policytype || '|' || distkey::text FROM gp_distribution_policy WHERE localoid = 'dfe_fixture.${relation_name}'::regclass;")"
  if [[ "${canonical_distribution_policy}" != 'p|1' ]]; then
